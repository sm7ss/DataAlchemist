from src.strategies.pre_processing_strategies import NumericNulls, CategoricNulls
...
class CatNullExpr: 
    def __init__(self, model: BaseModel):
        self.null_cat = model.null_general_values.null_impute_categorics
    ...
    def input_mode(self, cat_col: str) -> pl.Expr: 
        return pl.col(cat_col).fill_null(pl.col(cat_col).mode().first())
...
class NumNullExpr: 
    def __init__(self, model: BaseModel):
        self.null_num = model.null_general_values.null_impute_numerics
...
class Nulls: 
    ...
    def analyse(self, col: str) -> Union[pl.Expr, str, None]: 
        ...
        if percent_nulls < self.model_threshold.columns_percent: 
            ...
            if percent_null_rows < self.model_threshold.rows_percent: 
                ...
                return [expr, index_rows, 'delete_rows']
        else:
    ...
    def fit_expressions(self, x_train: pl.DataFrame) -> pl.DataFrame:
        ...
        for expr in expressions: 
            valor= x_train.select(expr).item(0,0)
            col= expr.meta.root_names()[0]
            
            self.impute_values[col]= valor
        
        x_train= x_train.with_columns(expressions)
        logger.info('Nulls expressions were applied')
