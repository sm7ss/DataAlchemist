from .general_info import EdaGeneralInfo
from .null_val import EdaNullValues
from .statistics import AnalysisData
...
class SummaryInfo: 
    ...
    def duplicated(self) -> None: 
        duplicates= self.eda['general_eda']['duplicated']
        
        if duplicates > 0: 
            self.priority+= f'    - Handle duplicated rows\n'
    ...
    def summary(self, duplicated: bool=False, null: bool=False, outlier: bool=False, correlation: bool=False, categoric: bool=False) -> Dict[str, str]: 
        if duplicated: 
            self.duplicated()
...
========================================================================================'''
        ...
        summary= self.summary.summary(
            duplicated=True if duplicated else False,
            null= null_summary, 
...
class EdaPipeline: 
    def __init__(self,frame: pl.DataFrame, config: BaseModel):
        ...
        self.eda_null= EdaNullValues(frame=frame, config=config)
        self.eda_analysis= AnalysisData(
            frame= frame, 
            analysis= self.config_eda.basic_analysis_data, 
            config_threshold_eda= self.threshold_eda
        )
    ...
    def pipeline_eda(self) -> Dict[str, Path]: 
        ...
        enable_duplicated_analysis= self.config_eda.duplicated_values
        report= ''
        ...
        report= InfoAnalysisEda(
            ...
            duplicated=enable_duplicated_analysis,
